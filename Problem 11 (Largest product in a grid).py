...
def max_prod_slice(arr):
    left_diagonal_slice = np.prod([arr[i, i] for i in range(len(arr))])
    right_diagonal_slice = np.prod([arr[i, len(arr) - (i + 1)] for i in range(len(arr))])
    vertical_slices = max([np.prod(arr[0 : len(arr), i]) for i in range(len(arr))])
    horizontal_slices = max([np.prod(arr[i]) for i in range(len(arr))])
